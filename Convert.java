...
public final class Convert {
    ...
    public static Date toDate(Object value) {
        ...
        try {
            ...
            if (num == null) {
                if (str.length() < 11) {
                    time = FORMAT_DATE.get().parse(str);
                } else {
                    if (str.length() < 20) {
                        time = FORMAT_DATE_TIME.get().parse(str);
                    } else {
                        time = FORMAT_DATE_TIME_MS.get().parse(str);
                    }
                }
            } else {
                time = new Date(num);
        ...
        try {
            ...
            if (num == null) {
                if (str != null && str.length() < 11) {
                    timeFlag = FORMAT_DATE.get().parse(str).getTime();
                } else {
                    if (str != null && str.length() < 20) {
                        timeFlag = FORMAT_DATE_TIME.get().parse(str).getTime();
                    } else {
                        timeFlag = FORMAT_DATE_TIME_MS.get().parse(str).getTime();
                    }
                }
            } else {
                timeFlag = num;
    ...
    public static <T> T toType(Object value, Class<T> clazz) {
        ...
        try {
            ...
            return invoke(value, uncapitalize(shortClassName) + "Value");
    ...
    public static Date defaultDate(Object value) {
        return defaultDate(value, new Date());
    }

    public static Timestamp defaultTimestamp(Object value) {
        return defaultTimestamp(value, new Timestamp(System.currentTimeMillis()));
    }
    ...
    /**
     * 把Object转成BigDecimal再转成Double，如果是null则返回null.
     *
     * @param obj
     * @return
     */
    public static Double objToDecimalToDouble(Object obj) {
        ...
        return toDecimal(obj).doubleValue();
    }

    /**
     * 把 BigDecimal 类型转化成 String，保留 2 位小数.
     *
     * @param decimal
     * @return
     */
    ...
    /**
     * 金额输出
     * 把Object转成BigDecimal再转成string
     *
     * @param obj
     * @param
     * @return
     */
    public static String objToAmtString(Object obj) {

        BigDecimal decimal = Convert.defaultDecimal(obj, DECIMAL_DEFAULT);
        return Convert.decimalToString(decimal);
    }
     ...
     *
     * @param obj
     * @param defaultValue defaultValue 默认值： BigDecimal.ZERO(默认)
     * @return
     */
    public static String objToAmtString(Object obj, BigDecimal defaultValue) {

        BigDecimal decimal = Convert.defaultDecimal(obj, Convert.defaultDecimal(defaultValue));
        return Convert.decimalToString(decimal);
    }

    /**
     * 把 Double 类型转化成 String，保留 2 位小数.
     *
     * @param num
     * @return
     */
